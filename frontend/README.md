...
Each generated project includes a CLI tool named `wireframe`. Run commands from the `backend/` directory:

```bash
cd backend

# Server commands
uv run wireframe server run --reload     # Start dev server
uv run wireframe server routes           # Show all routes
# Database commands
uv run wireframe db migrate -m "message" # Create migration
uv run wireframe db upgrade              # Apply migrations
uv run wireframe db downgrade            # Rollback migration
# User commands
uv run wireframe user create-admin       # Create admin user
uv run wireframe user create             # Create regular user
uv run wireframe user list               # List all users
```
...
# Server
uv run wireframe server run --reload
uv run wireframe server routes

# Database (Alembic wrapper)
uv run wireframe db init
uv run wireframe db migrate -m "Add users"
uv run wireframe db upgrade

# Users
uv run wireframe user create-admin       # Create admin (interactive)
uv run wireframe user create             # Create user (interactive)
uv run wireframe user list               # List all users
```
...
```bash
uv run wireframe cmd seed --count 100
uv run wireframe cmd seed --dry-run
```
...
| Start dev server | `uv run --directory backend wireframe server run --reload` |
| Start prod server | `uv run --directory backend wireframe server run --host 0.0.0.0 --port 8080` |
| Show routes | `uv run --directory backend wireframe server routes` |
...
| Task | Command |
|------|---------|
| Create migration | `uv run --directory backend wireframe db migrate -m "message"` |
| Apply migrations | `uv run --directory backend wireframe db upgrade` |
| Rollback migration | `uv run --directory backend wireframe db downgrade` |
| Show current | `uv run --directory backend wireframe db current` |
| Show history | `uv run --directory backend wireframe db history` |

### Users

| Task | Command |
|------|---------|
| Create admin | `uv run --directory backend wireframe user create-admin` |
| Create user | `uv run --directory backend wireframe user create` |
| List users | `uv run --directory backend wireframe user list` |

### Taskiq

| Task | Command |
|------|---------|
| Start worker | `uv run --directory backend wireframe taskiq worker` |
| Start scheduler | `uv run --directory backend wireframe taskiq scheduler` |
